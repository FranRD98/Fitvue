...
<template>
  <div>
    ...
    <div v-else-if="routines.length === 0">No hay rutinas aún.</div>

    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      <div
        v-for="routines in routine"
        :key="routine.id"
      ...
      >
        ...
        <div class="mt-4">
          <router-link :to="`/rutinas/${routine.id}`" class="text-[var(--color-accent)] font-medium">
            Ver rutina →
          </router-link>
          <router-link
            v-if="isAdmin"
            :to="`/dashboard/rutinas/editar/${routine.id}`"
